...
function makeRequest(slug: string, sessionCookie?: string): NextRequest {
  ...
  return new NextRequest(`https://cristianbarbosa.test/api/admin/qr/${slug}/stickers`, { headers });
}

describe("GET /api/admin/qr/[slug]/stickers", () => {
  ...
  it("returns a downloadable sticker-sheet PNG for a real, logged-in-only request", async () => {
    process.env.ADMIN_SESSION_SECRET = TEST_SESSION_SECRET;
    const token = createSessionToken(TEST_SESSION_SECRET);
    ...
    expect(response.headers.get("content-disposition")).toContain("qr-stickers-colegio-la-leticia-2026.png");
