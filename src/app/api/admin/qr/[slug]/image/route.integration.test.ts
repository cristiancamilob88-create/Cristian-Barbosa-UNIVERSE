...
function makeRequest(slug: string, sessionCookie?: string): NextRequest {
  ...
  return new NextRequest(`https://cristianbarbosa.test/api/admin/qr/${slug}/image`, { headers });
}

describe("GET /api/admin/qr/[slug]/image", () => {
  ...
  it("returns a downloadable PNG for a real, logged-in-only request against a seeded slug", async () => {
    process.env.ADMIN_SESSION_SECRET = TEST_SESSION_SECRET;
    const token = createSessionToken(TEST_SESSION_SECRET);
    ...
    expect(response.headers.get("content-disposition")).toContain("qr-colegio-la-leticia-2026.png");

    const buf = Buffer.from(await response.arrayBuffer());
    // PNG magic bytes — proves QRCode actually rendered an image, not just set headers.
