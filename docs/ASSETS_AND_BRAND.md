...
|---|---|---|---|---|
| 1 | Logo principal | **REAL** — 2026-08-25, puesto en el sitio 2026-08-28 | El logo real ("Cristian Barbosa — La Piedra Preciosa", marca CB en diamante, fondo nebulosa rojo/azul) llegó y se usó para derivar el favicon (abajo). A pedido de Cristian ("empezar a acomodarlo" en las partes vacías), ahora está puesto: el emblema cuadrado (`icon.png`) junto al wordmark en `Header`/`Footer`, y el logo completo (`cristian-logo-01.png`) como marca de agua al 8% de opacidad en el hero de la homepage — misma técnica ya aprobada en `/bienvenida/[slug]`. | `Header.tsx`, `Footer.tsx`, `src/app/page.tsx` |
| 2 | Logo secundario (ícono/monograma) | **REAL** — 2026-08-25 | Recortada la marca CB del logo principal y generado `favicon.ico`/`icon.png`/`apple-icon.png` — ya no es el triángulo de Vercel. | `src/app/favicon.ico`, `src/app/icon.png`, `src/app/apple-icon.png` |
| 3 | Fotos de Cristian | **REAL** (parcial) — 2026-08-25, ampliado 2026-08-28 | `cristian-hero-02.jpg` en `/bienvenida/[slug]`. Nueva: `cristian-mountain-flex.jpg` (foto real, solo, sin nadie más en cuadro — sin pregunta de consentimiento) como hero de `/about`. `/entrenar` y la home (pillar grid) siguen en texto puro. | `/bienvenida/[slug]` hero, `/about` hero (listos); `/entrenar`, homepage pillar grid (pendientes) |
...
| 5 | Fotos de shows | **REAL** — 2026-08-28 | Tres fotos reales de shows/eventos de Cristian (`show-fan-lift-01.jpg`, `show-crowd-01.jpg`, `show-crowd-02.jpg`) en una galería "En acción". Consentimiento confirmado directamente con Cristian por cada foto antes de publicar: la amiga de la foto del evento fitness dio su ok: los asistentes (incluidos menores) dieron su consentimiento en el show, con sus padres presentes. Además, `cristian-brand-art-01.png` (arte de marca generado con IA, confirmado por Cristian — no una foto documental) como fondo del hero de `/shows`. | `/shows` hero + "En acción" |
...
| 14 | Videos | **REAL** (parcial) — 2026-08-25 | Un video real embebido en `/bienvenida/[slug]` vía `InstagramEmbed` (iframe directo al `/embed` de Instagram, sin SDK ni vendor nuevo — evita el problema del archivo original de +20 MB). El archivo original nunca se subió al repo/hosting propio; sigue sin existir un vendor de video propio para el día en que haga falta uno (docs/ARCHITECTURE.md §10-11). | `/bienvenida/[slug]` (listo); otras rutas, pendiente |
